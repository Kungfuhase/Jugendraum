...
def ui_board(board):
    print("   1   2   3   4   5   6   7")
    print(" |", board[0][0], "|", board[0][1], "|", board[0][2], "|", board[0][3], "|", board[0][4], "|", board[0][5],
          "|", board[0][6], "|")
    print(" |", board[1][0], "|", board[1][1], "|", board[1][2], "|", board[1][3], "|", board[1][4], "|", board[1][5],
          "|", board[1][6], "|")
    print(" |", board[2][0], "|", board[2][1], "|", board[2][2], "|", board[2][3], "|", board[2][4], "|", board[2][5],
          "|", board[2][6], "|")
    print(" |", board[3][0], "|", board[3][1], "|", board[3][2], "|", board[3][3], "|", board[3][4], "|", board[3][5],
          "|", board[3][6], "|")
    print(" |", board[4][0], "|", board[4][1], "|", board[4][2], "|", board[4][3], "|", board[4][4], "|", board[4][5],
          "|", board[4][6], "|")
    print(" |", board[5][0], "|", board[5][1], "|", board[5][2], "|", board[5][3], "|", board[5][4], "|", board[5][5],
          "|", board[5][6], "|")
...
def row_check(board, column):
    row_possible = 5
    for i in range(len(
            board)):  # checks which row is possible for placing an token\player symbol in this column. by checking the index (column) element of every row
        if board[i][column - 1] == " ":
            row_possible = i
...
def check_4_same(board, player_symbol):
    four_same = False
    for i in range(len(
            board)):  # überprüft ob vier aufeinanderfolgende elemente in einer list gleich sind fängt in der obersten an
        for j in range(len(board[i])):
            if (len(board[
                        i])) - j >= 4:  # Solange Elemente von j bis j+4 exitiert tue x (abruch falls nach elemten j nicht mehr 3 weitere kommen / nicht mehr 4 elemente möglich sind)
                if board[i][j:j + 4] == [player_symbol, player_symbol, player_symbol,
                                         player_symbol]:  # noch nach besserer methode für player_symbol fragen
                    four_same = True
    return four_same
...
def check_4_same_list(list, player_symbol):
    ...
    for i in range(len(list)):
        if (len(list)) - i >= 4:
            if list[i:i + 4] == [player_symbol, player_symbol, player_symbol,
                                 player_symbol]:  # noch nach besserer methode für player_symbol fragen
                four_same = True
    return four_same
...
def main():
    ...
    while True:
        ...
        ui_board(board_main)
        board_to_check = board_main
        print()

        if check_main(board_to_check, player):
