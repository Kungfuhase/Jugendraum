...
def check_4_same_list (list,player_symbol):
    ...
    for i in range(len(list)):
        if (len(list)) -i >= 4:
            if list[i:i+4] == [player_symbol,player_symbol,player_symbol,player_symbol]:
                four_same = True
    return four_same
...
def check_diag (board,player):
    ...
    loops = 0
    while loops != 3:
        for j in range(4): # nimmt aus jeder zeile die diagonale der ersten 4 elemente und packt sie zu einer liste
            pack = []
...
def check_main (board,player_symbol):
    board_in_check = board.copy()
    board_in_check_reversed = board.copy()
    results = []

    results.append (check_4_same(board_in_check,player_symbol))
    results.append (check_vert(board_in_check,player_symbol))

    if board_in_check [2] != [" "," "," "," "," "," "," "]:
        results.append(check_diag(board_in_check,player_symbol))
        for i in range(len(board_in_check_reversed)):
            board_in_check[i].reverse()
        results.append(check_diag(board_in_check_reversed,player_symbol))
    if True in results:
        return True
    else :
        return False
...
def main() :
    ...
    while True:
        ...
        ui_board(board_main)
        print()
        if check_main(board_main,player) == True :
